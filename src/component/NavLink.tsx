...
export default function NavLink() {
    const [isHov , setisHov] = useState(null);
  ...
  return (
    <div className="cursor-pointer flex flex-col items-start justify-start relative z-20" onClick={() => isClick ? setisClick(false) : setisClick(true) }>
        ...
        <div className={`p-3 rounded-md border-purpleN border absolute top-12 bg-black/90 ${isClick ? "flex" : "hidden"} flex-col gap-1 w-64`}>
            <Link to="/AnimeList">
            <div className={`w-full flex flex-col items-start justify-start gap-2 px-3 py-2 rounded-sm border ${isHov == 1 ? "bg-purpleN/20 border-white/70" : "bg-black/0 border-white/20"}`} onMouseEnter={() => setisHov(1)} onMouseLeave={() => setisHov(null)}>
            ...
            <div className={`w-full flex flex-col items-start justify-start gap-2 px-3 py-2 rounded-sm border ${isHov == 2 ? "bg-purpleN/20 border-white/70" : "bg-black/0 border-white/20"}`} onMouseEnter={() => setisHov(2)} onMouseLeave={() => setisHov(null)}>
                <p className='text-base'>Release lineup</p>
                <p className={`text-sm opacity-70 ${isHov == 2 ? "flex" : "hidden"}`}>Fresh Episode Drops</p>
            </div>
            </Link>
            <Link to="/HotLeaks">
            <div className={`w-full flex flex-col items-start justify-start gap-2 px-3 py-2 rounded-sm border ${isHov == 3 ? "bg-purpleN/20 border-white/70" : "bg-black/0 border-white/20"}`} onMouseEnter={() => setisHov(3)} onMouseLeave={() => setisHov(null)}>
