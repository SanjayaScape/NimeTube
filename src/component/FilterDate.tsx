    ...
    <div className={`flex flex-col items-start justify-start relative z-81 w-42 ${FO == "Date" || iHover ? "bg-purpleN/20 border-white/40" : "bg-purpleN/5 border-white/15"} rounded-md border `} onMouseEnter={() => {setiHover(true)}} onMouseLeave={() => {setiHover(false)}} onClick={ () => FO == "Date" ? ( setFO(""), console.log("a")): ( setFO("Date"), console.log("b"))}>
            <div className="flex flex-row items-center justify-between px-4 py-3 w-full">
                ...
                <div className={`flex items-center justify-center transition-all duration-200 ease ${ FO == "Date" ? "rotate-315" : "rotate-225"}`}>
                    <img className={`items-center justify-center transition-all duration-200 ease min-w-3.5 w-3.5 h-3.5 ${ FO == "Date" ? "hidden" : "flex"}`} src="/v_Block.svg" alt="" />
                    <img className={`items-center justify-center transition-all duration-200 ease min-w-4.5 w-4.5 h-4.5 rotate-45 ${ FO == "Date" ? "flex" : "hidden"}`} src="/v_Full.svg" alt="" />
                </div>
            </div>
            <div className={`${ FO == "Date" ? "flex" : "hidden"} flex-col absolute w-72 px-3 py-2 bg-black/97 top-14 z-65 right-0 gap-1 rounded-md overflow-y-scroll h-48`}>
                {years.map(y => (
                    <div className={`w-full flex flex-row items-center justify-center border border-white/10 ${iHoverD == y? "bg-purpleN/20 border-white/40" : "bg-purpleN/5"} px-3 py-2 rounded-md`} onMouseEnter={() => setiHoverD(y)} onMouseLeave={() => setiHoverD(0)} onClick={ () => isClickD != y ? (setisClickD(y), setY(isClickD as number) ) : (setisClickD("Release Year"), setY(0))} key={y}>
