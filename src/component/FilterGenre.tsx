...
export default function FilterGenre({setG , setTG , setFO , FO} : setP) {
    ...
    return (
    <div className={`flex flex-col items-start justify-start relative z-81 w-42 ${isClick || iHover ? "bg-purpleN/20 border-white/40" : "bg-purpleN/5 border-white/15"} rounded-md border `} onMouseEnter={() => {setiHover(true)}} onMouseLeave={() => {setiHover(false)}} onClick={ () => isClick ? (setisClick(false) , setFO("") ) : (setisClick(true) , setFO("Genre") )} >
        <div className="flex flex-row items-center justify-between px-4 py-3 w-full gap-2">
            <p className={`text-base ${iHover || (isClick && FO == "Genre") ? "opacity-100" : "opacity-70"} truncate`}>{isClickD}</p>
            <div className={`flex items-center justify-center transition-all duration-200 ease ${isClick  && FO == "Genre" ? "rotate-315" : "rotate-225"}`}>
                <img className={`items-center justify-center transition-all duration-200 ease min-w-3.5 w-3.5 h-3.5 ${isClick  && FO == "Genre" ? "hidden" : "flex"}`} src="/v_Block.svg" alt="" />
                <img className={`items-center justify-center transition-all duration-200 ease min-w-4.5 w-4.5 h-4.5 rotate-45 ${isClick && FO == "Genre" ? "flex" : "hidden"}`} src="/v_Full.svg" alt="" />
            </div>
        </div>
        <div className={`${isClick && FO == "Genre" ? "flex" : "hidden"} flex-col absolute w-72 px-3 py-2 bg-black/97 top-14 z-65 right-0 gap-1 rounded-md overflow-y-scroll h-48`}>
            {genre.map(gen => (
